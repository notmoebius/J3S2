...
def select_bonjour_monde_5handle(reporters)
  # sélectionne 5 items
  result = []
  5.times do
     #créer un tableau
    result << reporters[rand(reporters.count)]
  end
  return result
end

def send_tweet_to_handles(senders, post_tweet)
  #senders.each {| sender | puts "Salut #{sender} ! ceci est message d'un projet étudiant api twitter #bonjour_monde from @the_hacking_pro" }
  senders.each {| sender | post_tweet.update("Salut #{sender} ! ceci est message d'un projet étudiant api twitter #bonjour_monde from @the_hacking_pro") }
end

# main

# Mon tableau de journalistes
...
# prends 5 item du tableau au hasard
send_tweet = select_bonjour_monde_5handle(ary_handles)

# envoi le tweet au 5 reporters
send_tweet_to_handles(send_tweet,client_tweet)
